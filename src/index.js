...
import "./debug.js"; // Include debug functions
...
todoArea.addEventListener("click", (event) => {
  ...
  } else if (event.target.classList.contains("todotaskedit")) {
    const todotaskedit = new editTodo("todotaskedit", event.target);
    todotaskedit.todoTaskedit();
  }
});
...
projectArea.addEventListener("click", (event) => {
  ...
  } else if (event.target.classList.contains("projecttaskedit")) {
    const projectCard = event.target.closest(".ProjectCard");
    if (projectCard) {
      // const projectId = projectCard.id.replace("-card", "");
      // const projectButton = document.getElementById(projectId);
      const editProjecttask = new editTodo(projectCard.id, event.target);
      editProjecttask.projectTaskedit();
    }
  }
});
